...
```{r "chicks and weather graph fro total precip"}
...
```{r "loading eggs"}
dat<-read.csv("~/Desktop/fullsummary_complete.csv")
dat$d_FirstEgg<-as.Date(dat$d_FirstEgg, format="%m/%d/%Y")

eggs_data<-dat %>%    
  na.omit() %>% 
  mutate(lay_year = year(d_FirstEgg), 
         lay_month = month(d_FirstEgg)) %>% 
  group_by(lay_year, lay_month) %>%
  summarize(total_eggs = sum(NumEggsLaid)) %>%
  filter(lay_year != "2015")
eggs_data %>% drop_na(d_FirstEgg) %>% group_by(as.factor(year)) %>% tally()
```

```{r "joining for eggs_daylength"}
colnames(eggs_data)
colnames(daylength)
colnames(eggs_data)[1:2]<-c("year", "month")

eggs_daylength<- left_join(eggs_data,daylength) 

```

```{r "graph for eggs_daylength"}
g4<-ggplot(eggs_daylength, aes(x = month, y = mean_daylength , fill = year)) +
  geom_bar(aes( y= total_eggs, fill = year), stat = "identity", alpha= 0.5) +
  geom_line(aes(colour = year), alpha=0.5) +
  geom_point(aes(colour=year), alpha =0.5) +
  
    scale_x_continuous(breaks = c(1:12), labels = c("J","F","M","A","M","J","J","A", "S", "O", "N", "D")) +
    labs(title = "Total Eggs laid per Daylength - Davis, CA",
         subtitle = " Bars are Total Eggs Laid",
         y = "Average Daylength", x = "Month") + theme_bw(base_size = 15) +
     facet_wrap(vars(year), ncol=2)
g4
```

```{r "eggs_daylength stats"}
g4<-ggplot(eggs_daylength, aes(x = mean_daylength, y = total_eggs)) + 
  geom_point()+ 
  labs(title = "Total Eggs per Daylength",
       y="Total Eggs Laid", x="Daylength") +
  geom_smooth(method = lm)
g4
```

```{r "eggs_daylength cor.testion"}
cor.test(eggs_daylength$total_eggs, eggs_daylength$mean_daylength, method = "pearson")
cor.test(eggs_daylength$total_eggs, eggs_daylength$month, method = "pearson")
cor.test(eggs_daylength$total_eggs, eggs_daylength$year, method = "pearson")
```
